...
fn eq_in_val(e: &Expr, other: &Expr) -> bool {
    ...
    match (&e.item, &other.item) {
        ...
        (Loop { vars: xs, init: i1, body: e1 }, Loop { vars: ys, init: i2, body: e2 }) => {
...
fn conv(mut e: Box<Expr>, effects: &mut Set, saved: &mut Map) -> Box<Expr> {
    ...
    e.item = match e.item {
            ...
            let kind = match l {
                LetKind::Let(d, e1, e2) => {
                    ...
                    if let Some(x) = saved.get(&key) {
                        ...
                        let r = LetKind::Let(d, conv(e1, effects, saved), conv(e2, effects, saved));
        ...
        Loop { vars, init, body } => Loop { vars, init, body: conv(body, effects, saved) },
        _ => e.item
    };
