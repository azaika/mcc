...
fn is_tailrec(e: &Expr, f: &Id, is_inlet: bool, orig: &Vec<Decl>) -> (Option<BitVec>, bool) {
    ...
    match &e.item {
        ...
        If(_, x, y, e1, e2) => {
            if x != f && y != f {
                merge(
                    is_tailrec(&e1, f, is_inlet, orig),
                    is_tailrec(&e2, f, is_inlet, orig),
                )
            } else {
                (None, false)
            }
...
fn insert_continue(mut e: Box<Expr>, f: &Id, loop_vars: &Vec<Decl>, mask: &BitVec) -> Box<Expr> {
    ...
    e.item = match e.item {
        ...
        If(kind, x, y, e1, e2) => If(
            ...
            x,
            y,
            insert_continue(e1, f, loop_vars, mask),
            insert_continue(e2, f, loop_vars, mask),
        ),
        Let(d, e1, e2) => Let(d, e1, insert_continue(e2, f, loop_vars, mask)),
        LetRec(_, _) => panic!("LetRec should not appear in the loop"),
        Loop { .. } => {
