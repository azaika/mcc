...
fn conv(mut e: Box<Expr>, tyenv: &mut TyMap, consts: &mut ConstMap) -> Box<Expr> {
    ...
    e.item = match e.item {
        ...
        ExprKind::If(kind, x, y, e1, e2) => {
            ...
            let cond = match kind {
                IfKind::IfEq => xy_int.map(|(x, y)| x == y).or(xy_float.map(|(x, y)| x == y)),
                IfKind::IfLE => xy_float.map(|(x, y)| x <= y).or(xy_float.map(|(x, y)| x <= y)),
            };
        ...
        ExprKind::Loop { vars, init, cond, body } => ExprKind::Loop { vars, init, cond, body: conv(body, tyenv, consts) },
        _ => e.item
    };
