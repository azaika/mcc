...
fn insert_let<F: FnOnce(Id) -> (ExprKind, Ty)>(
    e: Expr,
    t: Ty,
    loc: util::Span,
    k: F,
...
fn conv(
    ...
    let (kind, t) = match e.item {
            ...
            if let Some(t) = env.get(&x) {
                ...
                if let Ty::Array(_) = t {
                    ...
                    return Err(anyhow::Error::msg(format!(
                        "external variable `{}` must have an array type, but it has type `{}`",
                        x, t
                    )));
                }
            }
        ...
        syntax::ExprKind::Let(l) => {
            ...
            match l {
                syntax::LetKind::Let(decl, e1, e2) => {
                    ...
                    (ExprKind::Let(decl_conv(decl), e1, e2), t2)
                }
        ...
        syntax::ExprKind::App(f, args) => {
            ...
            args.into_iter().rev().fold(make_app, |k, (ei, ti)| {
                Box::new(|mut vs| {
        ...
        syntax::ExprKind::Array(e1, e2) => {
            ...
            insert_let(*e1, t1, e.loc, |idx| {
                insert_let(*e2, t2, e.loc, |init| (ExprKind::CreateArray(idx, init), t))
            })
        }
        syntax::ExprKind::Get(e1, e2) => {
            ...
            insert_let(*e1, t1, e.loc, |arr| {
                insert_let(*e2, t2, e.loc, |idx| (ExprKind::ArrayGet(arr, idx), t))
            })
        }
