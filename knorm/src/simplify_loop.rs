...
fn emerge(e: &Expr, vars: &util::Set<Id>) -> bool {
    ...
    match &e.item {
        ...
        If(_, x, y, e1, e2) => vars.contains(x) || vars.contains(y) || emerge(e1, vars) || emerge(e2, vars),
...
fn has_only_exit(e: &Expr, vars: &mut util::Set<Id>) -> Option<bool> {
    ...
    match &e.item {
        ...
        _ => if emerge(e, &vars) { None } else { Some(true) }
    }
}
...
fn extract_tail(mut e: Box<Expr>, tyenv: &mut TyMap) -> Box<Expr> {
    ...
    let body = match &mut e.item {
        Loop { body, vars, .. } if has_only_exit(body, &mut vars.iter().map(|d| d.name.clone()).collect()).unwrap_or(false) => body,
        _=> return e
    };
...
fn conv(mut e: Box<Expr>, tyenv: &mut TyMap) -> Box<Expr> {
    ...
    e.item = match e.item {
        ...
        LetRec(Fundef { fvar, args, body }, e2) => {
            LetRec(Fundef { fvar, args, body: conv(body, tyenv) }, conv(e2, tyenv))
        },
        Loop { vars, init, body } => Loop{ vars, init, body: conv(body, tyenv) },
        e => e
    };
