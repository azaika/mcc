...
fn deref_ty(t: Ty) -> Ty {
    ...
    match t {
        Fun(args, ret) => Fun(
            args.into_iter().map(deref_ty).collect(),
            Box::new(deref_ty(*ret)),
        ),
    ...
    match &e.item {
        ...
        ExprKind::BinOp(op, e1, e2) => {
            ...
            match op {
                ...
                Eq | LE => {
                    unify(&infer_impl(e1, env, extenv)?, &infer_impl(e2, env, extenv)?)
                        .map_err(with(e.loc))?;
                    Ok(Ty::Bool)
                }
            }
        ...
        ExprKind::Let(l) => {
            ...
            match l {
                ...
                LetRec(fundef, e) => {
                    ...
                    let fty = Ty::Fun(
                        fundef.args.iter().map(|d| d.t.clone()).collect(),
                        Box::new(infer_impl(&fundef.body, env, extenv)?),
                    );
    ...
    let ek = match e.item {
        ...
        Let(l) => {
            let l = match l {
                ...
                LetKind::LetRec(Fundef { fvar, args, body }, e) => LetKind::LetRec(
                    Fundef {
                        fvar: deref_decl(fvar),
                        args: args.into_iter().map(deref_decl).collect(),
                        body: deref_term(*body),
                    },
                    deref_term(*e),
                ),
                LetKind::LetTuple(ds, e1, e2) => LetKind::LetTuple(
                    ds.into_iter().map(deref_decl).collect(),
                    deref_term(*e1),
                    deref_term(*e2),
                ),
            };
        ...
        App(f, args) => App(
            deref_term(*f),
            args.into_iter().map(|x| *deref_term(x)).collect(),
        ),
