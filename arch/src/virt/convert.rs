...
type ConstMap = util::Map<Id, closure::ConstKind>;
...
fn conv(e: Box<closure::Expr>, p: &mut Program, tyenv: &closure::TyMap, consts: &ConstMap) -> Box<Expr> {
    use ExprKind::*;
    ...
    match e.item {
        closure::ExprKind::Const(c) => {
            match c {
                ast::knormal::ConstKind::CUnit => lift(Nop),
                ast::knormal::ConstKind::CInt(x) => lift(Li(x)),
                ast::knormal::ConstKind::CFloat(x) => lift(FLi(x)),
            }
        },
        ...
        closure::ExprKind::Let(v, e1, e2) => {
            ...
            match &e1.item {
                closure::ExprKind::AllocArray(_, _, None) if !vt.is_pointer() => {
                    let e1 = lift(ExprKind::AllocHeap(Value::Imm(common::type_size(&vt) as i16)));
                ...
                closure::ExprKind::AllocArray(_, _, Some(init)) if !vt.is_pointer() => {
                    ...
                    for i in (0..num).rev() {
                        ...
                        e2 = lift(ExprKind::Let(None, lift(ExprKind::Sw(v.clone(), offset, init.clone())),e2));
                    }
                ...
                closure::ExprKind::MakeCls(label, args) => {
                    ...
                    for (idx, x) in args.iter().enumerate().rev() {
                        e2 = lift(ExprKind::Let(None, lift(ExprKind::Sw(v.clone(), Value::Imm((idx + 1) as i16), x.clone())), e2));
                    }
                    ...
                    let e1 = lift(ExprKind::AllocHeap(Value::Imm((1 + offsets.last().unwrap()) as i16)));
                    return lift(ExprKind::Let(Some(v), e1, e2));
                }
                _ => ()
            };
        ...
        closure::ExprKind::Tuple(xs) => {
            ...
            let name = gen_new_var(p, Ty::Tuple(xs.iter().map(|x| tyenv.get(x).unwrap().clone()).collect()));
            let mut e2 = lift(ExprKind::Var(name.clone()));
            for (offset, x) in offsets.into_iter().zip(xs).rev() {
                e2 = lift(ExprKind::Let(None, lift(ExprKind::Sw(name.clone(), Value::Imm(offset as i16), x)), e2));
            }
            let e2 = lift(ExprKind::Let(Some(name), lift(ExprKind::AllocHeap(Value::Imm(size as i16))), e2));

            e2
        },
        closure::ExprKind::CallDir(label, args) => lift(CallDir(label, args)),
        closure::ExprKind::CallCls(cls, args) => lift(CallCls(cls, args)),
        closure::ExprKind::AllocArray(num, _, None) => {
        ...
        closure::ExprKind::AllocArray(num, t, Some(init)) => {
            ...
            if let Some(num) = num_i {
                ...
                for i in (0..num).rev() {
                    ...
                    e2 = lift(ExprKind::Let(None, lift(ExprKind::Sw(name.clone(), offset, init.clone())),e2));
                }
            }
            else {
                ...
                let body = lift(ExprKind::If(
                    ...
                    var.clone(),
                    Value::Var(end.clone()),
                    lift(ExprKind::Sw(name.clone(), Value::Var(var.clone()), init.clone())),
                    lift(ExprKind::Nop)
                ));

                let lop = lift(ExprKind::Loop{vars: vec![var.clone()], init: vec![Value::Imm(0)], body });

                e2 = lift(ExprKind::Let(None, lop, e2));
                e2 = lift(ExprKind::Let(Some(end), lift(ExprKind::IntOp(IntOpKind::Sub, num.clone(), Value::Imm(1))), e2));
            }

            lift(ExprKind::Let(Some(name), lift(ExprKind::AllocHeap(Value::Var(num))), e2))
        },
        closure::ExprKind::ExtArray(label) => lift(GetLabel(label)),
        closure::ExprKind::ArrayGet(arr, idx) => {
            let t = tyenv.get(&arr).unwrap().elem_t();
            let size = type_size(t) as i32;
            if (t.is_array() || t.is_tuple()) && !t.is_pointer() {
                if let Some(ConstKind::CInt(idx)) = consts.get(&idx) {
                    ...
                    let offset = lift(ExprKind::IntOp(IntOpKind::Mul16, idx, Value::Imm(size as i16)));
                    let add = lift(ExprKind::IntOp(IntOpKind::Add, arr, Value::Var(offset_v.clone())));
        ...
        closure::ExprKind::DoAll { idx, range, body, .. } => lift(ExprKind::Loop{
            vars: vec![idx],
        ...
        closure::ExprKind::Assign(label, x) => {
            ...
            lift(ExprKind::Let(Some(lab.clone()), lift(GetLabel(label)), lift(ExprKind::Sw(lab.clone(), Value::Imm(0), x))))
        },
...
pub fn convert(cls: closure::Program, consts: &ConstMap) -> Program {
    ...
    for closure::Fundef { name, args, formal_fv, body } in cls.fundefs {
        let body = conv(body, &mut p, &cls.tyenv, consts);
        p.fundefs.push(Fundef { name, args, formal_fv, body });
    }
