...
pub fn prepare(p: &Program, var_idx: &util::Map<Var, usize>, def: &mut util::Set<LiveId>, used: &mut util::Set<LiveId>, follow: &mut util::Map<ProgramPoint, Follow>, prev: &mut util::Map<BlockId, Vec<ProgramPoint>>) {
    for bid in p.collect_used() {
        let block = &p.block_arena[bid];
        ...
        for (idx, (d, inst)) in block.body.iter().enumerate() {
            ...
            match &inst.item {
                ...
                IntOp(_, x, Value::Var(y)) | FloatOp(_, x, y) | Lw(x, Value::Var(y)) | Sw(x, _, y) => {
                    push(x);
                    push(y);
                }
                Mv(x) | UnOp(_, x) | IntOp(_, x, _) | AllocHeap(Value::Var(x)) | Lw(x, _) | Out(x) => push(x),
                CallDir(_, xs) => {
                    for x in xs {
                        push(x);
                    }
...
pub fn analyze(p: &Program, entry: BlockId, num_var: usize, vars: util::Map<Var, usize>, def: &util::Set<LiveId>, used: &util::Set<LiveId>, follow: &util::Map<ProgramPoint, Follow>, prev: &util::Map<BlockId, Vec<ProgramPoint>>) -> (Liveness, Liveness) {
    let mut live_in = util::Set::default();
    let mut live_out = util::Set::default();

    let mut queue = util::Set::with_capacity_and_hasher(p.block_arena.len() * 4, util::Hasher::default());
    for bid in p.collect_used_with(entry) {
        let block = &p.block_arena[bid];
    ...
    while !queue.is_empty() {
        ...
        if is_in {
            ...
            let b = match follow.get(&pp).unwrap() {
                Follow::Zero => false,
                Follow::One(p1) => {
                    live_in.contains(&(p1.clone(), i_v))
                },
                Follow::Two(p1, p2) => {
                    live_in.contains(&(p1.clone(), i_v)) ||
                    live_in.contains(&(p2.clone(), i_v))
                },
            };
